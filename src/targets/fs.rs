...
impl FileSystemBackupTarget {
    ...
    fn clone<T: BackupEntity>(&self, repo: &T, target: &Path, cancel: &AtomicBool) -> Result<String, errors::Error> {
    ...
    fn fetch<T: BackupEntity>(&self, repo: &T, target: &Path, cancel: &AtomicBool) -> Result<String, errors::Error> {
...
mod tests {
    ...
    async fn test_backup() {
        ...
        let id = agent.backup(&repo, &cancel).expect("initial backup to succeed (clone)");
        assert!(temp_dir.path().join(repo.backup_path()).join(".git").exists(), "the repository should have been created");

        let id2 = agent.backup(&repo, &cancel).expect("subsequent backup to succeed (fetch)");
